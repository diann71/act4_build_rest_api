...
userRouter.post("/login", async (req : Request, res : Response) => {
    try {
        ...
        if (!email || !password) {
            return res.status(StatusCodes.BAD_REQUEST).json({error : "Please providde all the required parameters.."})
        }
...
userRouter.put("/user/:id", async (req : Request, res : Response) => {
    ...
    try {
        ...
        if (!username || !email || !password) {
            return res.status(401).json({error : "Please providde all the required parameters.."})
        }
